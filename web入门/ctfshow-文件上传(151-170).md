...
![image-20220119181754788](image/ctfshow-文件上传(151-170)/image-20220119181754788.png)

![image-20220119181816409](image/ctfshow-文件上传(151-170)/image-20220119181816409.png)
...
![image-20220119190235209](image/ctfshow-文件上传(151-170)/image-20220119190235209.png)

![image-20220119190341168](image/ctfshow-文件上传(151-170)/image-20220119190341168.png)

![image-20220119190428130](image/ctfshow-文件上传(151-170)/image-20220119190428130.png)

## web168(基础免杀)

这个算是很简单的，同样是对上传的文件内容进行了限制，不允许上传一句话木马，如果只是为了拿到flag就不用考虑权限维持的问题，所以直接用命令读吧

![image-20220119193204715](image/ctfshow-文件上传(151-170)/image-20220119193204715.png)![image-20220119193227603](image/ctfshow-文件上传(151-170)/image-20220119193227603.png)
...
![image-20220119195037306](image/ctfshow-文件上传(151-170)/image-20220119195037306.png)

![image-20220119195117163](image/ctfshow-文件上传(151-170)/image-20220119195117163.png)

看到是可以包含的，那么接下来就是常规的写马，拿shell

![image-20220119195146942](image/ctfshow-文件上传(151-170)/image-20220119195146942.png)
...
文件上传结束了，学到了挺多有趣的知识，当然这些都是基本的，实战中想要实现还得看源码，之前挖洞的时候就遇到过一次文件上传getshell，是phpcms9.x的老洞了，到目前也只挖到过这么一个，还是太菜了，总之还是希望自己能变得更强一点吧。

——KonDream 2022年1月19日19:59:32(因疫情居家隔离中)
